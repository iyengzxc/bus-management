                        ...
                        <div class="mb-4">
                            ...
                            <select name="schedule_id" id="schedule_id" class="form-select mt-1 block w-full" required>
                                @foreach ($schedules as $schedule)
                                    <option value="{{ $schedule->id }}">{{ $schedule->bus->name }} - {{ $schedule->route->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            <label for="seat_number" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Seat Number</label>
                            <input type="number" name="seat_number" id="seat_number" class="form-input mt-1 block w-full" required>
                        </div>
